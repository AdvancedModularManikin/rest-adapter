...
std::vector<Assessment> AssessmentService::getAllAssessments() {
...
std::optional<Assessment> AssessmentService::getAssessment(const std::string& name) {
	std::lock_guard<std::mutex> lock(m_mutex);
...
void AssessmentService::createAssessment(const Assessment& assessment) {
	std::lock_guard<std::mutex> lock(m_mutex);
...
void AssessmentService::updateAssessment(const Assessment& assessment) {
	std::lock_guard<std::mutex> lock(m_mutex);
...
bool AssessmentService::deleteAssessment(const std::string& name) {
	std::lock_guard<std::mutex> lock(m_mutex);
...
void AssessmentService::sendPerformanceAssessment(
		const AMM::UUID& eventUUID,
...
std::filesystem::path AssessmentService::getAssessmentPath(const std::string& name) {
	return std::filesystem::path(Config::getInstance().getAssessmentPath()) / name;
}

void AssessmentService::ensureAssessmentDirectory() {
